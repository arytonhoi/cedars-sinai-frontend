...
class Announcement extends Component{
  render(){
    ...
    return (
      <label className={(what.isPinned)?("ann ann-pinned shadow"):("ann shadow")} htmlFor={index}>
        <div className={((now - what.createdTs) >7776000000)?("ann-old"):("")}>
              ...
              (<span>
                <label className="ann-menu" htmlFor={"conf_" + index}>
                  <svg className="ann-menu-icon" viewBox="0 0 120 120">
                   <line x1="10" y1="10" x2="100" y2="100" stroke="red" strokeWidth="10" />
                   <line x1="100" y1="10" x2="10" y2="100" stroke="red" strokeWidth="10" />
                  </svg>
                </label>
                <input type="checkbox" className="delete-confirm-toggle hide" id={"conf_" + index} />
                <label htmlFor={"conf_" + index} className="delete-confirm shadow hide">
                   <p
          ...
          <div className="ann-body hide">
            <div className="ann-text" dangerouslySetInnerHTML={{__html:what.content}}></div>
          </div>
        </div>
      </label>
    )
...
export default connect(mapStateToProps, { deleteAnnounce, clearErrors })(Announcement);
